...
class VolumeRenderer(object):
        ...
        if (nerf_input_feats is not None) and \
            ...
            if not only_render_background:
                output = self.forward_rendering(
                    H, output, fg_nerf, nerf_input_cams, nerf_input_feats, latent_codes, styles)
            if smooth_feat == True:
                output.reg_loss = self.get_smooth_loss(
                    H, fg_nerf, nerf_input_cams, nerf_input_feats, latent_codes, styles, output)
            elif smooth_feat_v2 == True:
                output.reg_loss = self.get_smooth_loss_v2(
                    H, fg_nerf, nerf_input_cams, nerf_input_feats, latent_codes, styles, output)
            elif smooth_feat_v3 == True:
                output.reg_loss = self.get_smooth_loss_v3(
                    H, fg_nerf, nerf_input_cams, nerf_input_feats, latent_codes, styles, output)
    ...
    def gen_optional_output(self, H, fg_nerf, nerf_input_cams, nerf_input_feats, latent_codes, styles, output):
        ...
        # print(camera_world[:, :1].shape)
        # print(camera_world[:, :1])
    ...
    # Add smooth loss
    # ----------------------------------------------------------------
    def get_smooth_loss(self, H, fg_nerf, nerf_input_cams, nerf_input_feats, latent_codes, styles, output):
        _, camera_world, ray_vector = nerf_input_cams
        z_shape_obj, z_app_obj = latent_codes[:2]
        fg_depth_map = torch.sum(output.fg_weights * output.fg_depths[1], dim=-1, keepdim=True)
        
        full_image = fg_depth_map * ray_vector
        img = camera_world[:, :1] + full_image

        img = img[:,:(H.tgt_res*H.tgt_res)]
        img = img.permute(0,2,1).reshape(-1, 3, H.tgt_res, H.tgt_res)
        ...
        shift_u, shift_d = img[:,:,:,2:], img[:,:,:,:-2]
        criterion = nn.L1Loss()
        loss_hor = criterion(shift_l, shift_r)
        loss_vert = criterion(shift_u, shift_d)
        loss = loss_hor + loss_vert
        return {'smooth_loss': loss}

    # ----------------------------------------------------------------
    # Add smooth loss (depth)
    # ----------------------------------------------------------------
    def get_smooth_loss_v2(self, H, fg_nerf, nerf_input_cams, nerf_input_feats, latent_codes, styles, output):
        _, camera_world, ray_vector = nerf_input_cams
        z_shape_obj, z_app_obj = latent_codes[:2]
        fg_depth_map = torch.sum(output.fg_weights * output.fg_depths[1], dim=-1, keepdim=True)
        img = fg_depth_map[:,:(H.tgt_res*H.tgt_res)]
        img = img.permute(0,2,1).reshape(-1, H.tgt_res, H.tgt_res) 
        if 'input_feats' in H.render_option:
            a, b = [r.split(':')[1:] for r in H.render_option.split(',') if r.startswith('input_feats')][0]
            a, b = int(a), int(b)
            if nerf_input_feats[0] == 'volume':
    ...
    def get_smooth_loss_v3(self, H, fg_nerf, nerf_input_cams, nerf_input_feats, latent_codes, styles, output):
        _, camera_world, ray_vector = nerf_input_cams
        z_shape_obj, z_app_obj = latent_codes[:2]
        fg_depth_map = torch.sum(output.fg_weights * output.fg_depths[1], dim=-1, keepdim=True)
        full_image = fg_depth_map * ray_vector
        ...
        n_shift_l, n_shift_r = img[:,:,2:,:], img[:,:,:-2,:]
        n_shift_u, n_shift_d = img[:,:,:,2:], img[:,:,:,:-2]
        loss_n_hor = criterion(n_shift_l, n_shift_r)
        loss_n_vert = criterion(n_shift_u, n_shift_d)
        loss += 0.5*(loss_n_hor+loss_n_vert)
        return {'smooth_loss': loss}
...
class Upsampler(object):
    ...
    def build_network(self, w_dim, input_dim, **block_kwargs):
        ...
        if len(self.block_resolutions) > 0:  # nerf resolution smaller than image
            ...
            for ir, res in enumerate(self.block_resolutions):
                ...
                # channels change by the resolution
    ...
    def forward_network(self, token_embeddings,assist_token_embeddings,assist_alpha,blocks, blocks_attn,blocks_offset,block_ws, x, img, target_res, alpha, skip_up=False, _lambda=0.5,is_rand = False,delta_w = None,**block_kwargs):
        imgs = []
        if delta_w == None:
            delta_w = []
        for index_l, (res, cur_ws) in enumerate(zip(self.block_resolutions, block_ws)):
            ...
            # TODO add delta to the cur_ws
            if is_rand == False:
                block_attn = blocks_attn[index_l]
                block_offset = blocks_offset[index_l]
                fusion_feature = block_attn(x,token_embeddings)
                w_delta = block_offset(fusion_feature)
                if assist_token_embeddings is not None:
                    assist_fusion_feature = block_attn(x,assist_token_embeddings)
                    assist_w_delta = block_offset(assist_fusion_feature)
                    w_delta = (1-assist_alpha)*w_delta + assist_alpha*assist_w_delta
                delta_w += [w_delta]
                w_delta = w_delta.repeat(1,cur_ws.shape[1],1)
                cur_ws = cur_ws + self._lambda * w_delta
            else:
                w_delta = delta_w[index_l]
                w_delta = w_delta.repeat(1,cur_ws.shape[1],1)
                cur_ws = cur_ws + self._lambda * w_delta
            x, img  = block(
                x, 
...
class NeRFSynthesisNetwork(torch.nn.Module):
    def __init__(self,
        ...
        **block_kwargs,            # Other arguments for SynthesisBlock.
        ...
        upsampler_kwargs.update(dict(
            ...
            in_res=resolution_vol, # 32
            out_res=img_resolution, # 512
            channel_max=channel_max, #1024
        ...
        if len(upsamplers) > 0:
            ...
            for u in upsamplers:
                ...
                setattr(self, u['name']+'_attn', u['attn'])
                setattr(self,u['name']+'_offset', u['offset'])
        # data-sampler
    ...
    def forward(self, ws,token_embeddings,assist_token_embeddings = None,assist_alpha=0.5,delta_w = None, **block_kwargs):
        ...
        # volume rendering
        # maybe contain NeRF path regularization
        ...
        if (cur_resolution > self.resolution_vol) or self.progressive_nerf_only:
            ...
            with torch.autograd.profiler.record_function('upsampling'):
                ...
                blocks_attn = [getattr(self, name+'_attn') for name in self.block_names]
                block_offset = [getattr(self, name+'_offset') for name in self.block_names]
                block_ws = self.U.forward_ws_split(ws, blocks)               
                u_imgs,delta_w = self.U.forward_network(token_embeddings,assist_token_embeddings,assist_alpha,blocks,blocks_attn,block_offset, block_ws, x, img, target_res, self.alpha, is_rand = False,**block_kwargs)
                imgs += u_imgs
                img      = imgs[-1]
                if len(rand_imgs) > 0:   # nerf path regularization
                    r_img, _ = self.U.forward_network(
                        token_embeddings,assist_token_embeddings,assist_alpha,blocks, blocks_attn,block_offset,block_ws, x_rand, img_rand, target_res, self.alpha, skip_up=True, is_rand = True,delta_w = delta_w,**block_kwargs)
                    rand_imgs += r_img
            ...
            block_kwargs['delta_w'] = delta_w
            delta_w_t = torch.cat(delta_w,dim = 0)
            # reg_loss['norm_delta_loss'] = torch.mean(torch.norm(delta_w_t, p=2, dim=-1))
            reg_loss['norm_delta_loss'] = torch.mean(torch.abs(delta_w_t))
        if len(reg_loss) > 0:
            for key in reg_loss:
...
class Discriminator(torch.nn.Module):
    def __init__(self,
        t_dim,                          # Conditioning label (C) dimensionality.
        ...
        **unused
        ...
        if self.t_dim > 0:
            self.mapping = MappingNetwork_4_text(z_dim=0, t_dim=self.t_dim, w_dim=cmap_dim, num_ws=None, w_avg_beta=None, **mapping_kwargs)
        ...
        # final output module
        self.b4 = DiscriminatorEpilogue(channels_dict[4], cmap_dim=0, resolution=4, **epilogue_kwargs, **common_kwargs)
        # matching loss
        self.b4_text = DiscriminatorEpilogue(channels_dict[4], cmap_dim=cmap_dim, resolution=4, **epilogue_kwargs, **common_kwargs)
        self.register_buffer("alpha", torch.scalar_tensor(-1))
    def set_alpha(self, alpha):
        if alpha is not None:
    ...
    def forward(self, inputs, t=None, aug_pipe=None, return_camera=False, **block_kwargs):
        ...
        if (t.size(-1) == 0) and self.predict_camera and (not self.separate_camera):
            t = self.projector(x)[:,:,0,0]
            if self.predict_9d_camera:
                t = camera_9d_to_16d(t)
            if self.predict_3d_camera:
                camera_loss = self.get_camera_loss(RT, UV, t)
                
        # camera conditional discriminator (disabled)

        # TODO change to text
        cmap = None
        if self.t_dim > 0:
            tt = t.clone().detach()
            tmap = self.mapping(None, tt)
        gen_logits  = self.b4(x, img, None)
        match_logits = self.b4_text(x,img,tmap)
        if self.dual_discriminator:
            logits = torch.cat([logits, self.b4(x_nerf, img_nerf, cmap)], 0)
                
        outputs = {'gen_logits': gen_logits,'match_logits':match_logits}
