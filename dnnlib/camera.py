...
def to_sphere(u, v, angular=False):
    ...
    if not angular:
        theta = 2 * math.pi * u # [0,2pi]
        phi = T.arccos(1 - 2 * v) #[0,pi]
    else:
        theta, phi = u, v
...
# sample the point from the pre-defined distribution
...
def look_at(eye, at=np.array([0, 0, 0]), up=np.array([0, 0, 1]), eps=1e-5,
            to_pytorch=True):
    # eye is the watching point
    # at is the target point (face to)
    if not isinstance(eye, torch.Tensor):
        ...
        # a vector that points to target from the eye point
