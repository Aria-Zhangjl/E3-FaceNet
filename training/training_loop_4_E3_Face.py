...
from training.data_utils import save_image_grid, resize_image, save_eval_caption
...
def setup_snapshot_image_grid(training_set, random_seed=0):
    ...
    gw = 6
    gh = 5
    # gw = np.clip(training_set.image_shape[2]*6 // training_set.image_shape[2], 7, 32)
    ...
    # Load data.
    images, texts, _ = zip(*[training_set[i] for i in grid_indices])
    texts = list(texts)
    return (gw, gh), np.stack(images),texts
...
def training_loop(
    ...
    smooth_feat             = False,    # Whether to smooth the NeRF representation
    smooth_feat_v2          = False,    # Whether to smooth the NeRF representation
    smooth_feat_v3          = False,    # Whether to smooth the NeRF representation
    **unused,
):
    # Initialize.
    ...
    # Add additional args (resolution and channel)
    ...
    t = torch.empty([batch_gpu, G.t_dim], device=device)
    t_token = torch.empty([batch_gpu,77,G.t_dim], device=device) # TODO 77 and G.t_dim is too experimentally
    img = misc.print_module_summary(G, [z, t,t_token], rank=rank)
    misc.print_module_summary(D, [img, t], rank=rank)
    ...
    # update loss name in run_train.py
    # G and D are all in loss
    loss = dnnlib.util.construct_class_by_name(device=device, **ddp_modules, **loss_kwargs) # subclass of training.loss.Loss

    phases = []
    for name, module, opt_kwargs, reg_interval in [('G', G, G_opt_kwargs, G_reg_interval), ('D', D, D_opt_kwargs, D_reg_interval)]:
        # construct Adam optimizer, update in run_train.py
    ...
    if rank == 0:
        ...
        # sample the true image from training dataset
        grid_size, images, texts = setup_snapshot_image_grid(training_set=training_set)
        grid_z = torch.randn([len(texts), G.z_dim], device=device).split(batch_gpu)
        grid_i = (torch.from_numpy(images).float() / 127.5 - 1).to(device).split(batch_gpu)
        # TODO deal with text condition
        texts_ids = clip.tokenize(texts).to(device)
        grid_t,grid_t_token = clip_model.encode_text(texts_ids)
        grid_t = grid_t.to(device).split(batch_gpu)
        grid_t_token = grid_t_token.to(device).split(batch_gpu)
        ...
        # generate fake image from g_ema
        if not os.path.exists( os.path.join(img_dir, 'fakes_init.png')):
            with torch.no_grad():
                images = torch.cat([G_ema.get_final_output(z=z, t = t,t_token = t_token,noise_mode='const', img=img).cpu() for z, img, t,t_token in zip(grid_z,  grid_i, grid_t,grid_t_token)]).numpy()
            save_image_grid(images, os.path.join(img_dir, 'fakes_init.png'), drange=[-1,1], grid_size=grid_size)
    
        # save real caption
        if not os.path.exists(os.path.join(img_dir, 'train_captions.txt')):
            save_eval_caption(texts, os.path.join(img_dir, 'train_captions.txt'))       
    # Initialize logs.
    if rank == 0:
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function('data_fetch'):
            def load_data(iterator):
                ...
                # TODO deal with text
                # Done!
                text = list(chunked(text,batch_gpu))
                return img, text
            phase_real_img, phase_real_t = load_data(training_set_iterator)
            all_gen_z   = torch.randn([len(phases) * batch_size, G.z_dim], device=device)
            all_gen_z   = [phase_gen_z.split(batch_gpu) for phase_gen_z in all_gen_z.split(batch_size)]
            # TODO deal with text conditioned
            all_gen_text   = [training_set._load_raw_text(np.random.randint(len(training_set))) for _ in range(len(phases) * batch_size)]
            all_gen_text = [tuple(chunked(phase_gen_text,batch_gpu)) for phase_gen_text in list(chunked(all_gen_text, batch_size))]
        ...
        for phase, phase_gen_z, phase_gen_text, phase_gen_img in zip(phases, all_gen_z, all_gen_text, all_gen_img):
            if batch_idx % phase.interval != 0:
                continue
            
            if generation_with_image:
                phase_gen_img, phase_gen_text = load_data(backup_data_iterator)
            ...
            for round_idx, (real_img, real_t, gen_z, gen_t, fake_img) in enumerate(zip(phase_real_img, phase_real_t, phase_gen_z, phase_gen_text, phase_gen_img)):
                sync = (round_idx == batch_size // (batch_gpu * world_size) - 1)
                gain = phase.interval
                # generate the image in loss.accumulate_gradients
                # each update one module (different from the other GAN implementation)
                # TODO add matching loss for text conditioned 
                losses = loss.accumulate_gradients(
                    phase=phase.name, 
                    real_img=real_img, 
                    real_t=real_t, gen_z=gen_z, 
                    gen_t=gen_t, fake_img=fake_img,
                    sync=sync, gain=gain, scaler=phase.scaler,
                    smooth_feat = smooth_feat,smooth_feat_v2 = smooth_feat_v2,smooth_feat_v3 = smooth_feat_v3)

            # Update weights. (G and D)
        ...
        # TODO save the captions and the generate image
        # Save image snapshot.
        if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
            with torch.no_grad():
                images = torch.cat([G_ema.get_final_output(z=z, t=t, t_token = t_token,noise_mode='const', img=None).cpu() for z, t, img,t_token in zip(grid_z, grid_t, grid_i,grid_t_token)]).numpy()
                save_image_grid(images, os.path.join(img_dir, f'fakes{cur_nimg//1000:06d}.png'), drange=[-1,1], grid_size=grid_size)

                images = torch.cat([G_ema.get_final_output(z=z, t=t, t_token = t_token,noise_mode='const', img=img, camera_mode=[0.5,0.5,0.5]).cpu() for z, t, img, t_token in zip(grid_z, grid_t, grid_i,grid_t_token)]).numpy()
        ...
        if (snapshot_data is not None) and (len(metrics) > 0) and (cur_tick > 1):
            ...
            for metric in metrics:
                result_dict = metric_main_delta.calc_metric(metric=metric, G=snapshot_data['G_ema'],
                    dataset_kwargs=training_set_kwargs, num_gpus=world_size, rank=rank, device=device,clip_model = clip_model)
                if rank == 0:
                    metric_main_delta.report_metric(result_dict, run_dir=run_dir, snapshot_pkl=snapshot_pkl)
