...
def setup_eval_loop_kwargs(cfg):
    ...
    if cfg.metrics is None:
        cfg.metrics = ['fid30k_full','clipsim_30k']
    ...
    args.update({"eval_set_kwargs": dict(class_name='training.dataset.ImageFolderDataset_4_mmceleba_text', data_dir=cfg.data, image_folder=cfg.image_folder,resolution=cfg.resolution, use_labels=False, max_size=None, xflip=False,types='test')})
    ...
    try:
        eval_set = dnnlib.util.construct_class_by_name(**args.eval_set_kwargs) # subclass of training.dataset.Dataset
        args.eval_set_kwargs.resolution = eval_set.resolution                  # be explicit about resolution
        args.eval_set_kwargs.use_labels = eval_set.has_labels                  # be explicit about labels
        args.eval_set_kwargs.max_size = len(eval_set)                          # be explicit about dataset size
        desc = eval_set.name
        del eval_set # conserve memory
    ...
    if cfg.cond:
        if not args.eval_set_kwargs.use_labels:
            raise UserError('cond=True requires labels specified in dataset.json')
        desc += '-cond'
    else:
        args.eval_set_kwargs.use_labels = False

    if cfg.subset is not None:
        assert isinstance(cfg.subset, int)
        if not 1 <= cfg.subset <= args.eval_set_kwargs.max_size:
            raise UserError(f'subset must be between 1 and {args.eval_set_kwargs.max_size}')
        desc += f'-subset{cfg.subset}'
        if cfg.subset < args.eval_set_kwargs.max_size:
            args.eval_set_kwargs.max_size = cfg.subset
            args.eval_set_kwargs.random_seed = args.random_seed
    ...
    if cfg.mirror:
        desc += '-mirror'
        args.eval_set_kwargs.xflip = False
    ...
    if cfg.spec.name == 'auto':
        res = args.eval_set_kwargs.resolution
    ...
    args.update({"loss_kwargs": dict(class_name='training.loss_4_text.StyleGAN2Loss', r1_gamma=cfg.spec.gamma, **cfg.model.loss_kwargs)})
...
def subprocess_fn(rank, args):
    ...
    evaluation_loop_4_E3_Face.eval_loop(**args)
...
@hydra.main(config_path="conf", config_name="config_mmceleba_eval")
def main(cfg: DictConfig):
    
    outdir = cfg.outdir

    # Setup training options
    run_desc, args = setup_eval_loop_kwargs(cfg)
    ...
    print(f'Training data:      {args.eval_set_kwargs.data_dir}')
    print(f'Training duration:  {args.total_kimg} kimg')
    print(f'Number of images:   {args.eval_set_kwargs.max_size}')
    print(f'Image resolution:   {args.eval_set_kwargs.resolution}')
    print(f'Conditional model:  {args.eval_set_kwargs.use_labels}')
    print(f'Dataset x-flips:    {args.eval_set_kwargs.xflip}')
    print()
