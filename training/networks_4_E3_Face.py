...
class Attention(torch.nn.Module):
    def __init__(self,
        ...
        self.to_k = torch.nn.Linear(cross_attention_dim,self.inner_dim,bias = bias)
        self.to_v = torch.nn.Linear(cross_attention_dim,self.inner_dim,bias = bias)
        
        self.to_out = torch.nn.ModuleList([])
        self.to_out.append(torch.nn.Linear(self.inner_dim,query_dim,bias=out_bias))
    ...
    def get_attention_score(self,query,key,attention_mask = None):
        dtype = query.dtype
        if attention_mask is None:
    ...
    def forward(self,hidden_states,encoder_hidden_states,attention_mask = None,scale = 1.0):
        input_ndim = hidden_states.ndim
        if input_ndim == 4:
            # TODO check the image feat before send to the Transformer Block
        ...
        attention_probs = self.get_attention_score(query,key,attention_mask)
...
#----------------------------------------------------------------------------
# TODO Fit the dtype error Done!
@persistence.persistent_class
class TransformerBlock(torch.nn.Module):
    def __init__(self, dim,size, num_attention_heads, attention_head_dim, dropout, cross_attention_dim, attention_bias,use_fp16=False):
        super().__init__()
        self.fp_16 = use_fp16
        self.norm1 = torch.nn.LayerNorm([dim,size,size],elementwise_affine=True)
        self.attn = Attention(
            query_dim = dim,
        ...
        # self.norm2 = torch.nn.LayerNorm([dim,size,size],elementwise_affine=True)
        # self.fc = FullyConnectedLayer(dim,dim,activation = activation_fn)

    def forward(self,hidden_states,encoder_hidden_states,attention_mask: Optional[torch.FloatTensor] = None,):
        hidden_states = hidden_states.to(torch.float32)
        encoder_hidden_states = encoder_hidden_states.to(torch.float32)
        norm_hidden_states = self.norm1(hidden_states)
        attn_output = self.attn(norm_hidden_states,encoder_hidden_states = encoder_hidden_states,attention_mask = attention_mask)
...
class MappingBlock(torch.nn.Module):
    def __init__(self,
        in_channels,                        # Number of input feature dimension.
        in_res,
        out_dim,                       # Number of output feature dimension.
        activation_fn,
    ):
        super().__init__()
        ...
        self.mapping = FullyConnectedLayer(in_res*in_res,out_dim,activation = activation_fn)
    def forward(self, x):
        b,_,h,w = x.shape
        x = self.conv(x)
        x = x.view(b,-1,h*w).squeeze(1)
        x = self.mapping(x)
        x = x.unsqueeze(1)
        return x
...
@persistence.persistent_class
class MappingNetwork_4_text(torch.nn.Module):
    def __init__(self,
        z_dim,                      # Input latent (Z) dimensionality, 0 = no latent.
        t_dim,                      # Conditioning label (C) dimensionality, 0 = no label.
        ...
        **unused,
        ...
        if t_dim > 0:   # project label condition
            self.embed = FullyConnectedLayer(t_dim, z_dim)
    ...
    def forward(self, z=None, t=None, truncation_psi=1, truncation_cutoff=None, skip_w_avg_update=False, styles=None, **unused_kwargs):
        ...
        with torch.autograd.profiler.record_function('input'):
            ...
            if self.t_dim > 0:
                misc.assert_shape(t, [None, self.t_dim])
                y = normalize_2nd_moment(self.embed(t.to(torch.float32)))
                # TODO may change to x + y
...
class SynthesisBlock(torch.nn.Module):
    def __init__(self,
        ...
        **layer_kwargs,                     # Arguments for SynthesisLayer.
        ...
        if in_channels == 0:
            # in original? the learnable constant input
...
class Generator(torch.nn.Module):
    ...
    def forward(self, z=None, t=None, t_token=None, assist_t_token = None, assist_alpha = 0.5,styles=None, truncation_psi=1, truncation_cutoff=None, img=None, **synthesis_kwargs):
        # get w style
        if styles is None:
            ...
            if (self.encoder is not None) and (img is not None):  #TODO: debug
                ...
                ws = self.mapping(z, t, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff, **synthesis_kwargs)
        else:
            ws = styles
        # synthesize the image
        img = self.synthesis(ws,t_token,assist_t_token,assist_alpha,**synthesis_kwargs)
        return img
...
@persistence.persistent_class
class DiscriminatorEpilogue_4_text(torch.nn.Module):
    def __init__(self,
        in_channels,                    # Number of input channels.
        cmap_dim,                       # Dimensionality of mapped conditioning label, 0 = no label.
        resolution,                     # Resolution of this block.
...
class Discriminator(torch.nn.Module):   # The original StyleGAN2 discriminator
    def __init__(self,
        ...
        img_channels,                   # Number of input color channels.
        t_dim               = 512,      # Conditioning text (T) dimensionality
        architecture        = 'resnet', # Architecture: 'orig', 'skip', 'resnet'.
        ...
        # TODO add a text conditioned matching loss
        self.text_cond_net =  DiscriminatorEpilogue_4_text(channels_dict[4], text_dim=t_dim, resolution=4, **epilogue_kwargs, **common_kwargs)
    
    def forward(self, img, t, c=None, **block_kwargs):
        x = None
        ...
        if self.t_dim > 0:
            text_logits = self.text_cond_net(x,img,t)
        return x
